...
def test_clean_fabric_vn(dpg_service, vnc_api_client, make_vmi, project):
    ...
    vnc_api_client.get_vmi_in_vn.return_value = [vmi_1, vmi_2, vmi_3]

    affected_vpg = dpg_service.clean_fabric_vn("dvs-1", "dpg-1")

    expected_fq_name = project.fq_name + ["dvs-1_dpg-1"]
    assert vnc_api_client.get_vmi_in_vn.call_args[0][0] == expected_fq_name
    assert vnc_api_client.delete_vmi.call_args_list == [
        mock.call("vmi-1"),
        mock.call("vmi-2"),
        mock.call("vmi-3"),
    ]
    assert vnc_api_client.dettach_vmi_from_vpg.call_args_list == [
