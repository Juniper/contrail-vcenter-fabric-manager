...
def test_from_vmware_dpg(vmware_dpg):
    ...
    assert dpg_model.uuid == models.key_to_uuid("dvportgroup-1")
...
def test_to_vnc_vn(project):
    dpg_model = models.DistributePortGroupModel(
        uuid=models.key_to_uuid("dvportgroup-1"),
        name="dpg-1",
        dvs_name="dvs-1",
        vlan_id=5,
    )
    ...
    assert vnc_vn.uuid == models.key_to_uuid("dvportgroup-1")
