...
from cvfm import clients, monitors, controllers
...
def vnc_rabbit_client(config):
    return clients.VNCRabbitClient(config["rabbit_config"])
...
@pytest.fixture
def vmware_controller(synchronizer, update_handler, lock, event):
    controller = VMwareMockController(
        synchronizer, update_handler, lock, event
    )
    controller.sync()
...
class VMwareMockController(controllers.VMwareController):
    def __init__(self, synchronizer, update_handler, lock, event):
